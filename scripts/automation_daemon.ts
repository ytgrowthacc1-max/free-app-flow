...
async function refreshOAuthToken(): Promise<string | null> {
  ...
  if (!refreshToken) {
    console.error("[OAUTH] No refresh token found in settings or env. Cannot refresh OAuth token.");
    return null;
  }

  try {
    console.log("[OAUTH] Requesting new access token using refresh token...");
    ...
    if (newAccessToken && newRefreshToken) {
      console.log("[OAUTH] OAuth token refreshed successfully for bot @appdevelopment!");
...
async function getValidOAuthToken(): Promise<string | null> {
  const token = await getSetting("whop_oauth_token", process.env.WHOP_OAUTH_TOKEN || "");
  return token || null;
}

async function sendSupportMessage(channelId: string, content: string): Promise<any> {
  let token = await getValidOAuthToken();

  if (!token) {
    console.log("[OAUTH] No token found, attempting refresh...");
    token = await refreshOAuthToken();
  }

  if (!token) {
    throw new Error("[OAUTH] Cannot send message: No valid bot OAuth token available.");
  }

  console.log(`[DAEMON] Sending message as bot @appdevelopment to channel ${channelId}...`);
  ...
  if (res.status === 401) {
    console.log("[OAUTH] OAuth token expired (401). Refreshing token from Supabase settings...");
    const refreshed = await refreshOAuthToken();
    if (refreshed) {
      console.log("[OAUTH] Retrying message sending with fresh OAuth token...");
  ...
  if (!res.ok) {
    ...
    console.error(`[OAUTH] Failed to send message using bot OAuth token:`, errText);
